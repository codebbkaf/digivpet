...
enum TrainOutcome: Equatable {
    /// It trained: one training charge spent, and `strengthStat` went up by `gain`.
    case trained(gain: Int)
...
enum TrainingStart: Equatable {
    /// The round is paid for and may begin: one training charge was spent.
    case started
    /// The round never started, for a reason worth showing the user. Nothing was charged.
    case blocked(reason: String)
}

/// Training: spends a training charge to buy `strengthStat`, which is what steers a Digimon toward
/// the stronger evolution branches and (US-030) what it fights with.
///
/// Since US-177 the currency is a training charge — earned from active calories at
/// `ConsumptionConfig.kcalPerTrain` and banked per-Digimon on `GameState.trainCharges` — rather than
/// the Strength/Stamina energy US-108 spent. This is the exercise-to-training loop the whole
/// consumption wave is built on: the calories you burn are what let you train.
///
...
enum TrainAction {
    ...
    /// Why a session is refused when the Digimon has no training charge to spend. Names the remedy
    /// rather than only the refusal — a charge comes from burning active calories, so there is
    /// something to do about it. Mirrors `FeedAction`'s "no meat — go battle" affordance: an empty
    /// currency plus a one-line "how to earn more" is the whole story of a zero-charge tap.
    static let noChargeReason = "No training charge — go exercise."

    /// Enters a training round: checks eligibility, spends a training charge, and counts the session.
    ///
    /// Checks run asleep -> sick -> funds, so a sleeping Digimon that is also out of charges is
    /// reported as asleep: the state the user can do something about tonight, rather than the one
    /// they cannot.
    ///
    /// **The charge is taken HERE and never given back.** A minigame that ends in a `miss`, or is
    /// dismissed halfway, has still spent the training charge — otherwise walking out of a round the
    /// user was losing would be free, and the button would be a slot machine rather than a cost.
    ///
    ...
    static func begin(_ state: GameState, isAsleep: Bool) -> TrainingStart {
        ...
        // The training charge (US-177), asked before the session is counted so a broke Digimon is
        // never marked as having trained. Spent one at a time and never given back, exactly like the
        // battle charge — see `MainScreenModel.battle`.
        guard state.trainCharges > 0 else {
            return .blocked(reason: noChargeReason)
        }
    ...
    static func train(_ state: GameState, isAsleep: Bool,
        ...
        case .started:
            return .trained(gain: finish(state, result: result))
        }
    }
}
