...
final class UltimateSweepCToDTests: XCTestCase {
    ...
    func testTheUltimateBucketFellByExactlyThisStorysTwenty() {
        ...
        XCTAssertEqual(orphans.count, 80,
                       "100 Ultimate were edge-orphaned before this story and 80 after")
        XCTAssertEqual(orphans.filter { !jogressReachable.contains($0) }.count, 78,
                       "78 is the true remainder once the two Jogress results are set aside")
    ...
    func testTheSweepOpenedNoNewLines() {
        ...
        XCTAssertEqual(sizes["penc-nsp"], 41)
        XCTAssertEqual(sizes["dmc-v1"], 39)
        XCTAssertEqual(sizes["dmc-v4"], 31)
    ...
    func testNoPerfectThisStoryForkedOffersTwoEdgesOnOneEnergy() throws {
        ...
        // The five Perfects that carry three edges after this story. (Chimairamon was NOT forked
        // here — Chaosmon, which would have, is a Jogress result left unwired — so it stays at the
        // two edges US-163 gave it.)
        for parent in ["deathmeramon", "doruguremon", "knightmon", "meicrackmon", "pencme_andromon"] {
    ...
    func testTheTwentyOrphansThisStoryRemovedAreAllPlainRosterIds() throws {
        ...
        XCTAssertEqual(graph.nodes.count, 837, "817 before this story, 837 after it")
    ...
    func testWhatThisSweepHandsToTheRestOfTheUltimateRung() throws {
        ...
        XCTAssertEqual(stillOrphaned.filter { $0.stage == .ultimate }.count, 80,
        ...
        XCTAssertEqual(stillOrphaned.filter { $0.stage == .ultimate && !jogressReachable.contains($0.id) }.count,
                       78, "78 Ultimate truly owed once the two Jogress results are set aside")
