...
final class ChildSweepAToFTests: XCTestCase {
    ...
    func testTheSweepOpenedNoNewLines() {
        ...
        XCTAssertEqual(sizes["palmon"], 30, "US-159's Lilamon and Lilimon X, plus US-163's one Ultimate")
    }
    ...
    func testTheOnlyDeadEndsBelowUltimateAreTheOnesTheSweepsHaveOpened() throws {
        ...
        // **US-161 CLEARS SIX AND ADDS TWO BACK, AND THE TWO ARE THE SAME BILL US-160 PAID** — it
        // opened the Perfect rung on TWO lines that had none, `vital` (Kokeshimon and Tia Ludomon,
        // for Oboromon and RaijiLudomon) and `xros` (Shoutmon King, for both OmegaShoutmon), and
        // each needed a junk floor before its Champions could branch at all. Both floors are
        // line-scoped ALIASES for the reason US-160 recorded — no unused junk-flavoured Perfect
        // sheet is left in the pack — so neither removes an orphan. The other four leaves cleared
        // are Galgomon for Rapidmon, GeoGreymon for RizeGreymon, Kuwagamon X for BOTH Okuwamon,
        // and that is the whole of it. Net 77 -> 73.
        ...
        // **US-165 CLEARS ONE MORE**, the E-H Ultimate sweep: Pandamon, a bolded Saiyu-Warriors
        // parent of Erlangmon, gains its single `isDefault` climb. Net 60 -> 59.
        let clearedByUS165 = ["pandamon"]
        // **US-166 CLEARS ONE MORE**, the I-M Ultimate sweep: Karakurumon, a bolded parent of
        // Kaguyamon, gains its single `isDefault` climb. Net 59 -> 58.
        let clearedByUS166 = ["karakurumon"]
        let cleared = clearedByUS157 + clearedByUS158 + clearedByUS159 + clearedByUS160
            + clearedByUS161 + clearedByUS162 + clearedByUS163 + clearedByUS164 + clearedByUS165
            + clearedByUS166
