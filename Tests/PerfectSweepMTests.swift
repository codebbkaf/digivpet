...
final class PerfectSweepMTests: XCTestCase {
    ...
    func testThePerfectsMLeftAsLeavesAreTheDeadEndLedgersOwn() throws {
        ...
        // US-163 wired Mametyramon (-> Bagramon) and MegaloGrowmon (-> Breakdramon); US-164 wired
        // the last one, MetalGreymon X, which now climbs to Chaosdramon and also forks to
        // Chaosdramon V2, both cited on Wikimon. No M Perfect is a leaf any longer.
        let wiredBySweeps = ["mametyramon", "megalogrowmon", "metalgreymon_x"]
        XCTAssertEqual(leaves, [],
                       "the M Perfect leaves have moved without the ledger moving with them")
        for id in leaves + wiredBySweeps {
            XCTAssertFalse(graph.parents(of: id).isEmpty,
                           "\(id) is an orphan rather than a leaf, so it WAS in this story's scope")
        }
        for id in wiredBySweeps {
    ...
    func testEverySweptPerfectClimbsByOneGatedDefaultEdge() throws {
        ...
        // US-168, the S-Z Ultimate sweep, forked machgaogamon to three: Sakuyamon and Sakuyamon X
        // beside its AncientVolcamon climb.
    ...
    func testDiablomonGainedBothOfItsMissingRungsAtOnce() throws {
        ...
        // US-164 added the eponymous Diablomon and Diablomon X to the line's Ultimate rung, both
        // over Meicrackmon — the only drawable route onto the line, every cited parent being
        // idle-only or at the wrong stage.
        XCTAssertEqual(Set(ultimates.map(\.id)),
                       ["rasielmon", "raguelmon", "diablomon", "diablomon_x"])
    ...
    func testTheSweepOpenedNoNewLines() {
        ...
        XCTAssertEqual(sizes["dmc-v3"], 58, "MarinBullmon, Ryugumon and MetalPhantomon, plus US-163's one Ultimate")
        ...
        XCTAssertEqual(sizes["wanyamon"], 33, "MachGaogamon, plus US-161's RizeGreymon and Ravmon")
        XCTAssertEqual(sizes["penc-nsp"], 47, "MegaSeadramon X, plus US-161's both Panjyamon, plus US-163's one Ultimate")
        XCTAssertEqual(sizes["penc-me"], 75, "MetalMamemon X, plus US-161's both Okuwamon, RizeGreymon X and two Kuwagamon Megas, plus US-163's four Ultimates")
    ...
    func testTheTwentySixOrphansThisStoryRemovedAreAllPlainRosterIds() throws {
        ...
        XCTAssertEqual(graph.nodes(at: .ultimate).count, 236, "93 before this story, 138 after US-163")
    }
